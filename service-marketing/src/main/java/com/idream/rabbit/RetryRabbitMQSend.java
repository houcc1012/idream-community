...
public class RetryRabbitMQSend {
    ...
    private void retrySend(final String msgId) {
        ...
        if (record != null && record.getRetryNo() == 0) {
            //修改状态
            sendMqMessageRecordMapper.updateStatusById(SystemEnum.SendMqMessageStatus.SEND_FAIL.getCode(), record.getId());
                ...
                retryer.call(new Callable<Boolean>() {
                    ...
                    public Boolean call() throws Exception {
                        ...
                        if (retryNo > record.getRetryMaxNo()) {
                            logger.info("超过最大重试次数，重试停止，发送邮件通知人工处理! msgID = " + msgId);
                            sendMail(msgId);
                            return false;
                        }
                        //修改重试次数
                        ...
                        //重新发送消息
                        rabbitSendService.retrySendAward(record.getContent(), msgId);
